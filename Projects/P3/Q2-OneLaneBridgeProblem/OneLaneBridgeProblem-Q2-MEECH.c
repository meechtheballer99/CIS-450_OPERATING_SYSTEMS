...
//MONITOR DATA STRUCTURES AND SHARED VARIABLES//
...
int main(int argc, char* argv[]) {
    ...
    filePtr = fopen("bridge_output.txt", "w"); //may need to use fopen_s instead of fopen() because fopen() is the obselete (depractated) version of the function; fopen_s() is the "safe" and more portable version of the function
    if (filePtr == NULL) {
...
void ArriveBridge(int direc, int id) {
    ...
    pthread_mutex_lock(&mutex_BridgeAccess); //lock here so that arrivals of cars cannot be interrupted/skipped by another thread (car)
...
int is_Safe(int direc) { //no need to place locks in this function; every time a thread wakes up to call this function, that thread has a lock already from when it wakes up from a signal via the condition variable
    if (currentNumber == 0)
...
void WELCOME_MESSAGE(int argc, char* argv[]) {
  ...
  //console output:
    printf("WELCOME TO THE MULTITHREADED ONE-LANE BRIDGE CROSSING PROGRAM\nADAPTED BY DEMETRIUS JOHNSON\nFOR UM-DEARBORN CIS-450 WITH DR. JINHUA GUO\n");
    if (argc == 2)
        printf("NUMBER OF CARS SCHEDULED TO ARRIVE: %d (default == 50).\nRe-run program and enter one Command Line Parameter to change.\n", atoi(argv[1]));
    else
        printf("NUMBER OF CARS SCHEDULED TO ARRIVE: 50 (default == 50).\nRe-run program and enter one Command Line Parameter to change.\n");
    printf("Output also sent to bridge_output.txt file in directory of the executable.\n\n");
    //file output:
    fprintf(filePtr, "WELCOME TO THE MULTITHREADED ONE-LANE BRIDGE CROSSING PROGRAM\nADAPTED BY DEMETRIUS JOHNSON\nFOR UM-DEARBORN CIS-450 WITH DR. JINHUA GUO\n");
    if (argc == 2)
        fprintf(filePtr, "NUMBER OF CARS SCHEDULED TO ARRIVE: %d (default == 50).\nRe-run program and enter one Command Line Parameter to change.\n", atoi(argv[1]));
    else
        fprintf(filePtr, "NUMBER OF CARS SCHEDULED TO ARRIVE: 50 (default == 50).\nRe-run program and enter one Command Line Parameter to change.\n");
    fprintf(filePtr, "Output also sent to bridge_output.txt file in directory of the executable.\n\n");
}
