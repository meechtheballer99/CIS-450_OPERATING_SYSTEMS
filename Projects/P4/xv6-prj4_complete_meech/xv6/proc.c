...
scheduler(void)
{
  ...
  int total_tickets;
  cmostime(&systime); //sum of system time fields = lottery scheduler seed
  ...
  for(;;){
    ...
    //Randomnly generated winner range must be within sum of tickets of
    ...
    winner = rand() % (total_tickets + 1); //+1 since initally total could = 0
...
int
settickets(int number) //by Demetrius E Johnson for CIS450 w/Prof Guo
{
 struct proc *p = myproc();
